...
def _build_requests(store, ext, batch_size):
...
def main(write: bool):
    ...
    requests, meta = _build_requests(store, ext, cfg.extraction_batch_size)
    nreq = len(requests)
    print(f"Quarterly reports: {len(meta)} | batch requests: {nreq} | "
          f"mode: {'WRITE (submit batch)' if write else 'DRY-RUN (no submit, no spend)'}")
    ...
    print(f"Submitted batch {batch.id}; polling every {_POLL_SECONDS}s (usually < 1h)...")
    while True:
